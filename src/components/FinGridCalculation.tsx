...
const FilterForm = () => {
  ...
  const [marginal, setMarginal] = useState<number>(0.0);
  ...
  useEffect(() => {
    ...
    if (savedState) {
      ...
      setCsvFile(
        state.csvFile ? new File([state.csvFile], state.csvFile.name) : null
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (fixedPrice !== "" && csvFile) {
      ...
      try {
        ...
        if (data.MonthlyData) {
          const sortedMonthlyData = data.MonthlyData.sort(
            (a, b) => a.Year - b.Year
          );
  ...
  const getGraphData = () => {
    ...
    if (timePeriod === "day" && resultData.DailyData) {
      const days = resultData.DailyData.slice(
        currentDayIndex,
        currentDayIndex + maxItems
      );
      ...
      const weeks = resultData.WeeklyData.slice(
        currentWeekIndex,
        currentWeekIndex + maxItems
      );
      ...
      let months = resultData.MonthlyData.filter((data) =>
        isMobile() ? true : data.Year === currentYear
      ) // Handle year filter only if not mobile
      ...
      if (isMobile()) {
        ...
        if (isYearEnd) {
          ...
          const nextMonthData = resultData.MonthlyData.find(
            (data) => data.Year === nextYear && data.Month === 1
          );
          months = nextMonthData ? [nextMonthData] : []; // Set months to only include January of the next year
        } else {
          months = months.slice(
            currentMonthIndex,
            currentMonthIndex + maxItems
          );
        }
      } else {
      ...
      fixedPrices = months.map((data) =>
        parseFloat(data.FixedPrice.toFixed(2))
      );
  ...
  const getNextYearAvailable = () => {
    ...
    return (
      resultData?.MonthlyData?.some((data) => data.Year === currentYear + 1) ??
      false
    );
  };
    ...
    <Container className="filter-form-container">
      <div className="section">
        ...
        <div className="intro-text">
          ...
          <p>
            {t("setFixedPrice")} <b>snt/kwh.</b>
          </p>
          <p>
            {t("uploadAndViewResults")}
            <b>{t("uploadAndViewResultsBold")}</b>
          </p>
        </div>
      </div>
          ...
          <Col xs={12} md={6} className="mb-3">
            <Form.Group controlId="fixedPrice">
              ...
              <Form.Control
                ...
                value={fixedPrice}
                onChange={(e) =>
                  setFixedPrice(parseFloat(e.target.value) || "")
                }
              />
            </Form.Group>
          </Col>
          ...
          <Col xs={12} md={6} className="mb-3">
            <Form.Group controlId="marginal">
              ...
              <Form.Control
                ...
                value={marginal}
                onChange={(e) => setMarginal(parseFloat(e.target.value) || 0.0)}
              />
            </Form.Group>
          </Col>
        ...
        <>
          <div className="result-data-container">
            <div className="result-data-summary">
              ...
              <p>
                ...
                <b>
                  {resultData.StartDate
                    ? formatStartDateEndDate(resultData.StartDate)
                    : "N/A"}{" "}
                  -
                  {resultData.EndDate
                    ? formatStartDateEndDate(resultData.EndDate)
                    : "N/A"}
                </b>
              </p>
              <p>
              ...
              {resultData.CheaperOption === "SpotPrice" && (
                <p>
                  <b>{t("equilevantFixedPriceBold")}</b>
                  {t("equilevantFixedPrice")}
                  <b>
                    {" "}
                    {resultData.EquivalentFixedPrice?.toFixed(2)} {t("unit")}{" "}
                  </b>{" "}
                </p>
              )}
              <p className="price-difference">
                {t("priceDifference")}{" "}
                <b>{resultData.PriceDifference?.toFixed(2) ?? "N/A"} €</b>
              </p>
              <p className="price-difference">
                {t("optimizedPriceDifference")}{" "}
                <b>
                  {resultData.OptimizedPriceDifference?.toFixed(2) ?? "N/A"} €
                </b>
              </p>
              <OverlayTrigger
                placement="right"
            ...
            <div className="result-data-keywords">
              <p>
                ...
                <span className="dynamic-value">
                  {resultData.TotalConsumption?.toFixed(2) ?? "N/A"}
                </span>{" "}
                kWh
              </p>
              <p>
                ...
                <span className="dynamic-value">
                  {resultData.TotalSpotPrice?.toFixed(2) ?? "N/A"}
                </span>{" "}
                €
              </p>
              <p>
                ...
                <span className="dynamic-value">
                  {resultData.TotalFixedPrice?.toFixed(2) ?? "N/A"}
                </span>{" "}
                €
              </p>
              <p>
              ...
              <p>
                {t("optimizedSpotElectricityPrice")}{" "}
                <span className="dynamic-value">
                  {resultData.TotalOptimizedSpotPrice?.toFixed(2) ?? "N/A"}
                </span>{" "}
                €
              </p>
            </div>
          ...
          <div className="chart-controls">
            {timePeriod === "day" && (
              <div className="day-period-controls">
                <Button
                  onClick={handlePrevDayPeriod}
                  disabled={currentDayIndex === 0}
                >
                  {t("previous")}
                </Button>
                <Button
                  onClick={handleNextDayPeriod}
                  disabled={
                    currentDayIndex + (isMobile() ? 6 : 15) >=
                    (resultData.DailyData?.length || 0)
                  }
                >
                  {t("next")}
                </Button>
            ...
            {timePeriod === "week" && (
              <div className="week-period-controls">
                <Button
                  onClick={handlePrevWeekPeriod}
                  disabled={currentWeekIndex === 0}
                >
                  {t("previous")}
                </Button>
                <Button
                  onClick={handleNextWeekPeriod}
                  disabled={
                    currentWeekIndex + (isMobile() ? 6 : 15) >=
                    (resultData.WeeklyData?.length || 0)
                  }
                >
                  {t("next")}
                </Button>
                  ...
                  <>
                    ...
                    <Button
                      ...
                      disabled={
                        ...
                        (resultData?.MonthlyData?.length || 0)
                      }
                    >
                      {t("next")}
                    </Button>
